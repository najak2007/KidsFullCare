...
exports.searchSchool = onCall(
  ...
  async (request) => {
   ...
   const results = rows.map((row) => ({
      ...
      ORG_RDNMA: row.ORG_RDNMA === null ? "" : row.ORG_RDNMA,                     // 도로주소
      ORG_RDNDA: row.ORG_RDNDA === null ? "" : row.ORG_RDNDA,                     // 도로 상세주소
      ...
      SCHUL_KND_SC_NM: row.SCHUL_KND_SC_NM === null ? "" : row.SCHUL_KND_SC_NM,         // 초등학교/중학교/고등학교 등
      schoollevel: row.SCHUL_KND_SC_NM === "초등학교" ? "1" : "2",
      SCHUL_KND_SC_NM: row.SCHUL_KND_SC_NM === null? "" : row.SCHUL_KND_SC_NM,         // 학교종류명
      LCTN_SC_NM: row.LCTN_SC_NM === null ? "" : row.LCTN_SC_NM,                   // 시도명
      JU_ORG_NM: row.JU_ORG_NM === null ? "" : row.JU_ORG_NM,                     // 관할조직명
      HS_SC_NM: row.HS_SC_NM === null ? "" : row.HS_SC_NM,                       // 고등학교구분명
      FOND_SC_NM: row.FOND_SC_NM === null ? "" : row.FOND_SC_NM,                   // 설립명
      FOND_YMD: row.FOND_YMD === null ? "" : row.FOND_YMD,
      FOAS_MEMRD: row.FOAS_MEMRD === null ? "" : row.FOAS_MEMRD,                   // 
      ORG_TELNO: row.ORG_TELNO === null ? "" : row.ORG_TELNO,                           // 전화번호
    }));
